...
public class EstadoRegistro extends JFrame {
	...
	void listar() {
		...
		try {
			...
			Object[] EstadoRegistro= new Object[3];
			model = (DefaultTableModel)dataTable.getModel();
			
			while (rs.next()) {
			 EstadoRegistro[0] = rs.getInt("EstRegCod");
			 EstadoRegistro[1] = rs.getString("EstRegNom");
			 EstadoRegistro[2] = rs.getString("EstRegEstReg");
				model.addRow(EstadoRegistro);
			}
	...
	void agregar() {
		...
		if (descripcion.equals("") || estadoRegistro.equals("")) {
			...
			String sql = "INSERT INTO ESTADO_REGISTRO(EstRegCod, EstRegNom, EstRegEstReg)VALUES("+c+", '"+descripcion+"', '"+estadoRegistro+"')";
			try {
				cn = con.getConnection();
				st = cn.createStatement();
	...
	void modificar() {
		...
		String sql = "UPDATE ESTADO_REGISTRO SET EstRegNom='"+desc+"', EstRegEstReg='"+estReg+"' WHERE EstRegCod="+cod;
	...
	void eliminar() {
		...
		if (filaSeleccionada == -1) {
			...
			String sql = "DELETE FROM ESTADO_REGISTRO WHERE EstRegCod="+cod;
			try {
				cn = con.getConnection();
				st = cn.createStatement();
				st.executeUpdate(sql);
	...
	public EstadoRegistro() {
		...
		contentPane.setBorder(new TitledBorder(new LineBorder(new Color(184, 207, 229)), "ESTADO REGISTRO", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(51, 51, 51)));
		...
		panel_tabla.setBorder(new TitledBorder(new LineBorder(new Color(184, 207, 229)), "Tabla Estado Registro", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(51, 51, 51)));
