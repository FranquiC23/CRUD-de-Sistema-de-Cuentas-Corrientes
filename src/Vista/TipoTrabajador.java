...
public class TipoTrabajador extends JFrame {
	...
	void listar() {
		...
		try {
			...
			Object[] tipoTrabajador = new Object[3];
			model = (DefaultTableModel)dataTable.getModel();
			
			while (rs.next()) {
				tipoTrabajador[0] = rs.getInt("TipTraCod");
				tipoTrabajador[1] = rs.getString("TipTraNom");
				tipoTrabajador[2] = rs.getString("TipTraEstReg");
				model.addRow(tipoTrabajador);
			}
	...
	void agregar() {
		...
		if (descripcion.equals("") || estadoRegistro.equals("")) {
			...
			String sql = "INSERT INTO TIPO_TRABAJADOR(TipTraCod, TipTraNom, TipTraEstReg)VALUES("+c+", '"+descripcion+"', '"+estadoRegistro+"')";
			try {
				cn = con.getConnection();
				st = cn.createStatement();
	...
	void modificar() {
		...
		String sql = "UPDATE TIPO_TRABAJADOR SET TipTraNom='"+desc+"', TipTraEstReg='"+estReg+"' WHERE TipTraCod="+cod;
	...
	void eliminar() {
		...
		if (filaSeleccionada == -1) {
			...
			String sql = "DELETE FROM TIPO_TRABAJADOR WHERE TipTraCod="+cod;
			try {
				cn = con.getConnection();
				st = cn.createStatement();
				st.executeUpdate(sql);
	...
	void inactivar() {
		...
		if (filaSeleccionada == -1) {
			...
			String sql = "UPDATE TIPO_TRABAJADOR SET TipTraEstReg='I' WHERE TipTraCod="+cod;
			try {
				cn = con.getConnection();
				st = cn.createStatement();
				st.executeUpdate(sql);
	...
	void reactivar() {
		...
		if (filaSeleccionada == -1) {
			...
			String sql = "UPDATE TIPO_TRABAJADOR SET TipTraEstReg='A' WHERE TipTraCod="+cod;
			try {
				cn = con.getConnection();
				st = cn.createStatement();
				st.executeUpdate(sql);
	...
	public TipoTrabajador() {
		...
		contentPane.setBorder(new TitledBorder(new LineBorder(new Color(184, 207, 229)), "TIPO TRABAJADOR", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(51, 51, 51)));
		...
		panel_tabla.setBorder(new TitledBorder(new LineBorder(new Color(184, 207, 229)), "Tabla Tipo Trabajadores", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(51, 51, 51)));
