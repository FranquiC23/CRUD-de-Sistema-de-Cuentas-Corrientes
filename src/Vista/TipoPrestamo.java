...
public class TipoPrestamo extends JFrame {
	...
	void listar() {
		...
		String sql = "SELECT * FROM TIPO_PRESTAMO";
		try {
			cn = con.getConnection();
			st = cn.createStatement();
			rs = st.executeQuery(sql);
			Object[] tipoPrestamo= new Object[3];
			model = (DefaultTableModel)dataTable.getModel();
			
			while (rs.next()) {
				tipoPrestamo[0] = rs.getInt("TipPreCod");
				tipoPrestamo[1] = rs.getString("TipPreNom");
				tipoPrestamo[2] = rs.getString("TipPreEstReg");
				model.addRow(tipoPrestamo);
			}
	...
	void agregar() {
		...
		if (descripcion.equals("") || estadoRegistro.equals("")) {
			...
			String sql = "INSERT INTO TIPO_PRESTAMO(TipPreCod, TipPreNom, TipPreEstReg)VALUES("+c+", '"+descripcion+"', '"+estadoRegistro+"')";
			try {
				cn = con.getConnection();
				st = cn.createStatement();
	...
	void modificar() {
		...
		String sql = "UPDATE TIPO_PRESTAMO SET TipPreNom='"+desc+"', TipPreEstReg='"+estReg+"' WHERE TipPreCod="+cod;
	...
	void eliminar() {
		...
		if (filaSeleccionada == -1) {
			...
			String sql = "DELETE FROM TIPO_PRESTAMO WHERE TipPreCod="+cod;
			try {
				cn = con.getConnection();
				st = cn.createStatement();
				st.executeUpdate(sql);
	...
	void inactivar() {
		...
		if (filaSeleccionada == -1) {
			...
			String sql = "UPDATE TIPO_PRESTAMO SET TipPreEstReg='I' WHERE TipPreCod="+cod;
			try {
				cn = con.getConnection();
				st = cn.createStatement();
				st.executeUpdate(sql);
	...
	void reactivar() {
		...
		if (filaSeleccionada == -1) {
			...
			String sql = "UPDATE TIPO_PRESTAMO SET TipPreEstReg='A' WHERE TipPreCod="+cod;
			try {
				cn = con.getConnection();
				st = cn.createStatement();
				st.executeUpdate(sql);
	...
	public TipoPrestamo() {
		...
		contentPane.setBorder(new TitledBorder(new LineBorder(new Color(184, 207, 229)), "TIPO PRESTAMO", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(51, 51, 51)));
		...
		panel_tabla.setBorder(new TitledBorder(new LineBorder(new Color(184, 207, 229)), "Tabla Tipo Prestamo", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(51, 51, 51)));
