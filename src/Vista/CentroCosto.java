...
public class CentroCosto extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					CentroCosto frame = new CentroCosto();
	...
	void listar() {
		...
		try {
			...
			Object[] centroCosto= new Object[3];
			model = (DefaultTableModel)dataTable.getModel();
			
			while (rs.next()) {
				centroCosto[0] = rs.getInt("CenCosCod");
				centroCosto[1] = rs.getString("CenCosNom");
				centroCosto[2] = rs.getString("CenCosEstReg");
				model.addRow(centroCosto);
			}
	...
	void agregar() {
		...
		if (descripcion.equals("") || estadoRegistro.equals("")) {
			...
			String sql = "INSERT INTO CENTRO_COSTO(CenCosCod, CenCosNom, CenCosEstReg)VALUES("+c+", '"+descripcion+"', '"+estadoRegistro+"')";
			try {
				cn = con.getConnection();
				st = cn.createStatement();
	...
	void modificar() {
		...
		String sql = "UPDATE CENTRO_COSTO SET CenCosNom='"+desc+"', CenCosEstReg='"+estReg+"' WHERE CenCosCod="+cod;
	...
	void eliminar() {
		...
		if (filaSeleccionada == -1) {
			...
			String sql = "DELETE FROM CENTRO_COSTO WHERE CenCosCod="+cod;
			try {
				cn = con.getConnection();
				st = cn.createStatement();
				st.executeUpdate(sql);
	...
	void inactivar() {
		...
		if (filaSeleccionada == -1) {
			...
			String sql = "UPDATE CENTRO_COSTO SET CenCosEstReg='I' WHERE CenCosCod="+cod;
			try {
				cn = con.getConnection();
				st = cn.createStatement();
				st.executeUpdate(sql);
	...
	void reactivar() {
		...
		if (filaSeleccionada == -1) {
			...
			String sql = "UPDATE CENTRO_COSTO SET CenCosEstReg='A' WHERE CenCosCod="+cod;
			try {
				cn = con.getConnection();
				st = cn.createStatement();
				st.executeUpdate(sql);
	...
	public CentroCosto() {
		...
		contentPane.setBorder(new TitledBorder(new LineBorder(new Color(184, 207, 229)), "CENTRO COSTO", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(51, 51, 51)));
		...
		panel_tabla.setBorder(new TitledBorder(new LineBorder(new Color(184, 207, 229)), "Tabla Centro Costo", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(51, 51, 51)));
